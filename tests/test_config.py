#!/usr/bin/env python3
"""Tests for ohbs-host.toml config loading and CLI merging."""
...
def _args(**kwargs):
    ...
    # so that ohbs-host.toml can supply them.
    defaults = {
        "os": None,
        "profile": None,
        "platform": None,
...
def test_load_missing_file_returns_none(tmp_path):
    assert ohbs_host_config.load(str(tmp_path / "nope.toml")) is None


def test_load_valid_toml(tmp_path):
    p = tmp_path / "ohbs-host.toml"
    p.write_text("""
[profile]
os = "rhel9"
profile = "L2"
...
exclude = "5.1.1"
""")
    cfg = ohbs_host_config.load(str(p))
...
""")
    args = _args()
    merged = ohbs_host_config.merge(args, str(p))
...
def test_merge_without_config_returns_args(tmp_path):
    args = _args(os="rhel9")
    merged = ohbs_host_config.merge(args, str(tmp_path / "missing.toml"))
    assert merged.os == "rhel9"
