...
periodic watch session (ciscvm_diff.py) plus CLI integration.

Pure-logic tests import ciscvm_diff directly — no root, no engine, no
...
def test_diff_classifies_drift_recovery_and_still_fail(before_doc, after_doc):
    report = ciscvm_diff.diff_results(before_doc, after_doc)
    ch = report.changes

    assert {c.rule_id for c in ch[ciscvm_diff.NEW_FAIL]} == {"1.1.1.1", "1.1.1.5"}
    assert {c.rule_id for c in ch[ciscvm_diff.RECOVERED]} == {"1.1.1.2"}
    assert {c.rule_id for c in ch[ciscvm_diff.STILL_FAIL]} == {"1.1.1.3"}
    assert {c.rule_id for c in ch[ciscvm_diff.UNWAIVED]} == {"1.1.1.6"}
...
def test_diff_rule_change_severity_meta():
    r = ciscvm_diff.RuleChange("1.1.1.1", ciscvm_diff.NEW_FAIL, "fail", "pass",
                               "x", meta={"family": "file"})
...
def test_diff_waived_rule_does_not_count_as_drift(before_doc):
    ...
    report = ciscvm_diff.diff_results(before_doc, after)
    assert {c.rule_id for c in report.changes[ciscvm_diff.NOW_WAIVED]} == {"1.1.1.1"}
    assert report.changes[ciscvm_diff.NEW_FAIL] == []


def test_diff_handles_rules_only_on_one_side(before_doc):
    after = make_result([rule("1.1.1.1", "pass"), rule("9.9.9.9", "pass")], score=90.0)
    report = ciscvm_diff.diff_results(before_doc, after)
    assert {c.rule_id for c in report.changes[ciscvm_diff.AFTER_ONLY]} == {"9.9.9.9"}
    assert "1.1.1.2" in {c.rule_id for c in report.changes[ciscvm_diff.BEFORE_ONLY]}


def test_diff_warns_on_mismatched_targets(before_doc):
    other = make_result([rule("1.1.1.1", "pass")], os_name="ubuntu2204", score=80.0)
    report = ciscvm_diff.diff_results(before_doc, other)
...
def test_diff_sorting_is_stable():
    ...
    ids = [c.rule_id for c in report.changes[ciscvm_diff.NEW_FAIL]]
...
def test_verify_remediation_classifies_fixed_still_and_regressed():
    ...
    v = ciscvm_diff.verify_remediation(pre, post)
...
def test_verify_remediation_no_baseline_is_safe():
    v = ciscvm_diff.verify_remediation(None, make_result([rule("1.1.1.1", "fail")]))
...
def test_audit_waivers_statuses_and_catalog():
    ...
    entries = {e.rule_id: e for e in ciscvm_diff.audit_waivers(waivers, catalog)}
...
def test_waiver_problems_lists_expired_invalid_and_noop():
    ...
    problems = ciscvm_diff.waiver_problems(waivers, catalog_ids={"1.1.1.1", "1.1.1.2"})
    text = "\n".join(problems)
...
def test_waiver_problems_clean():
    ...
    assert ciscvm_diff.waiver_problems(waivers, {"1.1.1.1", "1.1.1.2"}) == []
    assert ciscvm_diff.waiver_problems(None) == []
    assert ciscvm_diff.waiver_problems("not json {") == []


# ─── renderers ────────────────────────────────────────────────────────

def test_render_cli_contains_drift_sections(before_doc, after_doc):
    out = ciscvm_diff.render_cli(ciscvm_diff.diff_results(before_doc, after_doc))
...
def test_render_html_has_filter_and_severity(before_doc, after_doc):
    out = ciscvm_diff.render_html(ciscvm_diff.diff_results(before_doc, after_doc),
...
def test_render_verify_html(before_doc):
    v = ciscvm_diff.verify_remediation(
        before_doc,
        make_result([rule("1.1.1.1", "pass"), rule("1.1.1.2", "pass"),
                     rule("1.1.1.3", "fail"), rule("1.1.1.4", "fail")], score=50.0))
    out = ciscvm_diff.render_verify_html(v, name="CIS RHEL 9", profile="L1")
...
def _events_for(results, fail_on=(), **kwargs):
    events = []
    ws = ciscvm_diff.WatchSession(make_scanner(results, fail_on),
...
def test_watch_explicit_baseline_skips_first_scan_for_diffing():
    ...
    drifted = make_result([rule("1.1.1.1", "fail")], score=50.0)
    events = []
    ws = ciscvm_diff.WatchSession(make_scanner([drifted]), max_runs=1, interval=0,
...
def test_watch_interrupt_stops_gracefully():
    base = make_result([rule("1.1.1.1", "pass")], score=100.0)
    events = []
    ws = ciscvm_diff.WatchSession(make_scanner([base]), max_runs=99, interval=0,
                                  on_event=events.append)
    original = ciscvm_diff.time.sleep
    ciscvm_diff.time.sleep = lambda _s: (_ for _ in ()).throw(KeyboardInterrupt())
    try:
        ws.run()
    finally:
        ciscvm_diff.time.sleep = original
...
def test_watch_alert_callback_fires_on_new_drift_only():
    ...
    alerts = []
    events = []
    ws = ciscvm_diff.WatchSession(make_scanner([base, drifted, drifted]),
...
def test_watch_json_events_are_line_json(capsys):
    base = make_result([rule("1.1.1.1", "pass")], score=100.0)
    drifted = make_result([rule("1.1.1.1", "fail")], score=50.0)
    ws = ciscvm_diff.WatchSession(make_scanner([base, drifted]), max_runs=2,
