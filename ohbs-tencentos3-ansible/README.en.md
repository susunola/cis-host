...
基于 CIS **TencentOS Linux 3 Benchmark v1.0.0** 的合规扫描与修复 Playbook。
...
```
  ...
  ansible-playbook
       ...
       │                                scan ：只读
       │                                apply：改配置 + 再校验
       │                                      备份到 /var/backups/cis-tencentos3/
       │                                   │
       │                                   ▼
       │                              result.json
       ◀────────────────────────────────
...
└── roles/ohbs-tencentos3/
    ├── defaults/main.yml          # 角色默认变量
    ├── vars/main.yml              # 内部变量
    ├── meta/main.yml              # 元数据
    ├── files/
    │   ├── ohbs_engine.py          # 合规评估引擎（Python 3）
    │   ├── rules.json             # 规则目录
...
评估引擎 (`ohbs_engine.py`) 是一个纯 Python 3 脚本（无第三方依赖），在目标机上以 root 身份运行。它实现了 **61 个检查族**和 **55 个自动修复族**，覆盖 CIS 基准中约 **95%** 的自动化判定规则。
...
2. `apply` 模式会修改系统配置文件（原文件备份至 `/var/backups/cis-tencentos3/`）
