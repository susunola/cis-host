...
# CIS TencentOS Linux 4 Benchmark v1.0.0 — 合规扫描与修复 Playbook
...
families**, covering about **88%** of the benchmark's automatable rules.
...
2. `apply` 模式会修改系统配置文件（原文件备份至 `/var/backups/cis-tencentos4/`）· `apply` edits configs (backups in `/var/backups/cis-tencentos4/`)
