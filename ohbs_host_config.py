#!/usr/bin/env python3
"""Load and merge ohbs-host.toml configuration with CLI arguments."""
...
def load(path):
    """Load a ohbs-host.toml file and return a normalized dict.
    ...
    if tomllib is None:
        raise RuntimeError(
            "ohbs-host.toml requires Python 3.11+ or `pip install tomli`"
        )
...
def merge(args, config_path=None):
    """Merge ohbs-host.toml defaults into argparse args object.
    ...
    if config_path is None:
        config_path = os.environ.get("OHBS_HOST_CONFIG", "ohbs-host.toml")
